...
##### 三、锁
1. mysql中的锁
当数据库中有并发事务时，可能会产生数据的不一致，这时候需要一些机制来保证访问的次序，锁就是这样的一个机制。

2. mysql中的锁
- 从锁的类别上来说有共享锁和排他锁
- 从所得粒度来说有行锁和表锁
- 共享锁：读锁，当读数据时，对数据加共享锁。共享锁可以同时加多个。
- 排他锁：写锁，当写数据时，对数据加排他锁。排他锁只能加一个，并且与其他的排他锁和共享锁互斥。

3. mysql中innodb的行锁时怎么实现的
innodb时基于索引来完成行锁的，如select * from test where id = 1 for update.for update可以根据条件来完成对行的锁定，并且id是有索引键的列，如果id不是索引键那么innodb会完成表锁。

4. innodb存储引擎的锁的算法
- record lock:单个行记录上的锁
- gap lock ：间隙锁，锁定一个范围，不包括记录本身
- next-key lock：临键锁，锁定一个范围，包括记录本身
- 注意：当查询的索引含有唯一属性时，临键锁将会降级成行锁。

5. 什么是死锁，如何解决
- 死锁：两个或多个事务对同一资源的相互占用，并请求锁定对方的资源，从而导致恶行循环的现象。
- 解决办法：
- 1. 如果不同程序会并发存取多个表，尽量约定以相同的顺序访问表，可以大大降低死锁概率
- 2. 在同一个事务中，尽可能做到一次锁定所需要的所有资源，减少死锁产生概率
- 3. 对于非常容易产生死锁的业务部分，可以尝试通过表级锁来减少死锁概率
- 4. 可以用分布式事务锁或者乐观锁

6. 数据库中的乐观锁和悲观锁
- 乐观锁：假设多个事务进行操作时不会发生并发冲突，只是在提交操作时检查是否违反数据完整性。在修改数据时通过version把事务锁起来。实现方式有两种：cas和版本号机制。适用于读多写少的场景。
- 悲观锁：假设会发生并发冲突，所以屏蔽一切可能违反数据完整性的操作。实现机制：使用数据库中的锁机制。使用与写多读少的场景。
...
2. 大表数据查询，怎么优化
- 优化结构，sql查询语句，加合适的索引
- 增加缓存层，如memcache,redis
- 主从复制，读写分离
- 垂直拆分，分局模块的耦合度，将一个大的系统拆分成多个小的系统，也就是分布式系统
- 水平切分，针对数据量大的表，进行合理的分表

3. 主键使用自增id还是uuid
推荐使用自增i的，在innodb中，主键索引时聚簇索引，即主键索引的B+树的叶子节点按照顺序存储了主键索引及全部数据，如果主键时自增id，则对于插入操作只需要不断向后排列即可，但是如果使用uuid,对于下一条数据来说，和原来的不确定那个大小，所以会造成很多的数据插入和移动，产生很多内存碎片，从而导致插入的性能下降。

4. mysql主从复制
- 1. 定义：将主数据库中的ddl和dml操作通过二进制日志传输到从数据库，从数据库将这些日志重新执行，从而是得主数据库和从数据库的数据保持一致
- 主从复制的作用：主数据库出现问题时，可以切换到从数据库；可以读写分离；可以在从数据库上做日产备份
- 主从复制过程：1. 在主库山把数据更新记录到二进制文件中 2. 从库将主库的日志复制到自己的中继日志 3. 从库读取中继日志的时间，将其重放到从库数据中


5.mysql如何保证数据的一致性和持久性
- 保证一致性：在数据库层面，数据库通过原子性、隔离性和持久性来保证的一致性。
- 原子性是利用的undo log,即回滚日志，当事务执行失败或者调用了rollback,则可以利用undo log来进行回滚。
- 持久性是利用redo log。当数据修改时，不仅在内存中操作，还会记录到redo日志中，当事务提交时，会将redo日志进行刷盘。
